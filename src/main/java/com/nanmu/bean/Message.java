package com.nanmu.bean;

import com.alibaba.fastjson.JSONObject;
import com.nanmu.data.send.WeatherType;
...
public class Message {
    ...
    private void smType(String type, HashMap<String, ArrayList> smsType) {
...
//            if(weatherType.get(i).get("name").equals(type)){
            if(weatherType.get(i).getName().equals(type)){
                index = i;
                break;
            }
        }
        if(index == -1){
            ...
            weatherType.setName(type);
            weatherType.setValue(1);
...
//            Integer count = (Integer) weatherType.get(index).get("value");
            int value = weatherType.get(index).getValue();
            weatherType.get(index).setValue(value+1);
//            weatherType.get(index).put("value",count+1);
    ...
    public List<Integer> getTempType() {
        return tempType;
    }
    ...
    public void setWeatherType(List<WeatherType> weatherType) {
